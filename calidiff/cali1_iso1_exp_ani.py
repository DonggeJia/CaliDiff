...
# ============================================================
# 2. CalibDiff model: patch embedding + latent Transformer + 2D classifier
# ============================================================
...
class CalibDiff(nn.Module):
    ...
    def forward(self, I_k, I_goal, tau_k):
        ...
        # Offsets in [-0.5, 0.5] in bin-width units
...
def train_epoch(
    ...
    device: torch.device,
    log_every: int = 50,
    curve_path: str = "train_loss_curve2_local_exp_ani.png",
):
    model.train()
    total_loss = 0.0
    n_batches = 0
...
def main():
    root_dir = "/data/IMcali/sim_results_2025-12-19 18:26:15_exp_ani"  # TODO: set your dataset path
    img_size = 128
    patch_size = 2
    batch_size = 16
    num_epochs = 100
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        plot_all_losses(
            ...
            val_loss_hist,
            pde_rgb_mse_hist,
            out_path="loss_curves_all_local_exp_aniso.png",
        )

    torch.save(model.state_dict(), "calibdiff_polynomial_anisotropic_checkpoint.pt")
    print("Training complete, model saved to calibdiff_polynomial_anisotropic_checkpoint.pt")
    np.savez(
        "loss_histories_all_local_exp_aniso.npz",
