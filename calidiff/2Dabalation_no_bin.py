...
# -------------------------------------------------------------------------
# 2. CalibDiff model with CLIP language conditioning (CONTINUOUS 2D param)
# -------------------------------------------------------------------------
...
class CalibDiff(nn.Module):
    def __init__(
        ...
        theta_max=10.0,
        n_bins=8,           # (kept for structural similarity; unused in continuous head)
        lang_dim: int = 512,
    ):
        """
        CalibDiff with an extra CLIP language token, for 2D anisotropic params.

        ABLATION: Replace bin+offset (classification+regression) head with a
        continuous 2D regressor that directly predicts [a_x, a_y] in [theta_min, theta_max].
        """
        super().__init__()
        self.img_size = img_size
        self.patch_size = patch_size
        ...
        self.n_bins = n_bins  # unused in this ablation (kept to minimize diffs)
        ...
        # -----------------------------------------------------------------
        # ABLATED HEAD:
        #   Old: logits_x/offsets_x/logits_y/offsets_y (4 * n_bins outputs)
        #   New: raw continuous 2D output, squashed to [theta_min, theta_max]
        # -----------------------------------------------------------------
        self.head = nn.Sequential(
            nn.LayerNorm(embed_dim),
            nn.Linear(embed_dim, 2),  # [a_x_raw, a_y_raw]
        )
    ...
    def forward(self, I_k, I_goal, tau_k, lang_emb):
        ...
        Returns:
            a_hat:    [B,2] continuous predicted parameters [a_x, a_y]
                      constrained to [theta_min, theta_max]
        """
        B = I_k.shape[0]
        ...
        # continuous head
        raw_2d = self.head(z_cur_pool)           # [B,2]
        # squash to [theta_min, theta_max] (stable + hard range constraint)
        a_hat = self.theta_min + (self.theta_max - self.theta_min) * torch.sigmoid(raw_2d)
        return a_hat
...
def eval_epoch_pde_multi(
    ...
    for fam, sim_idx in traj_keys:
        ...
        for k in range(K):
            ...
            a_hat = model(I_k, I_goal, tau_k, lang_emb_fam)  # [1,2]
            a_hat_np = a_hat[0].detach().cpu().numpy()

            ax_hat_list.append(float(a_hat_np[0]))
            ay_hat_list.append(float(a_hat_np[1]))
...
# -------------------------------------------------------------------------
# 4. Training & validation loops (anisotropic, + language) — CONTINUOUS LOSS
# -------------------------------------------------------------------------
...
def eval_epoch(
    ...
    for batch in dataloader:
        ...
        a_hat = model(I_k, I_goal, tau_k, lang_batch)  # [B,2]

        loss = F.mse_loss(a_hat, target_params)
...
def main():
    ...
    theta_min = 1.0
    theta_max = 10.0
    n_bins = 8  # unused in continuous ablation, kept to minimize diffs
    ...
    model = CalibDiff(
        ...
        theta_min=theta_min,
        theta_max=theta_max,
        n_bins=n_bins,      # unused in continuous ablation
        lang_dim=lang_dim,
    ).to(device)
    ...
    torch.save(model.state_dict(), "calibdiff_multi_ani_clip_checkpoint.pt")
    print("Training complete, model saved to calibdiff_multi_ani_clip_checkpoint.pt")

    np.savez(
        "loss_histories_all_multi_ani_clip.npz",
