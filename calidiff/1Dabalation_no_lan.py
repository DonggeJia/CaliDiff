...
# -------------------------------------------------------------------------
# 2. CalibDiff model WITHOUT language conditioning (ABLATION)
# -------------------------------------------------------------------------
...
class CalibDiff(nn.Module):
    def __init__(
        ...
        """
        CalibDiff ablation WITHOUT CLIP language token.
...
# -------------------------------------------------------------------------
# 3. Multi-family PDE worker & evaluation (NO language)
# -------------------------------------------------------------------------
...
def eval_epoch_pde_multi(
    ...
    """
    Multi-family physics-based evaluation (NO language).

    For each trajectory (family, sim_dir), we:
      - predict a_hat_global(t) at all intervals k (no language input)
...
# -------------------------------------------------------------------------
# 4. Training & validation loops (same loss as your 1D scripts, NO language)
# -------------------------------------------------------------------------
...
def train_epoch(
    ...
    device: torch.device,
    log_every: int = 50,
    curve_path: str = "train_loss_curve_multi_iso_concrete_nolang.png",
):
    model.train()
    total_loss = 0.0
    n_batches = 0
    ...
    for step, batch in enumerate(dataloader, start=1):
        ...
        logits, offsets = model(I_k, I_goal, tau_k)
...
def plot_all_losses(
    ...
    val_losses,
    pde_losses,
    out_path: str = "loss_curves_all_multi_iso_concrete_nolang.png",
):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss / MSE")
    plt.title("Training / Validation / PDE Loss Curves (multi-family, NO language)")
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
# -------------------------------------------------------------------------
# 5. Main: multi-family / cross-family training + evaluation (NO language)
# -------------------------------------------------------------------------
...
def main():
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        plot_all_losses(
            ...
            val_loss_hist,
            pde_rgb_mse_hist,
            out_path="loss_curves_all_multi_iso_concrete_nolang.png",
        )

    torch.save(model.state_dict(), "calibdiff_multi_iso_concrete_nolang_checkpoint.pt")
    print("Training complete, model saved to calibdiff_multi_iso_concrete_nolang_checkpoint.pt")

    np.savez(
        "loss_histories_all_multi_iso_concrete_nolang.npz",
