...
# -------------------------------------------------------------------------
# 0. Simple self-trained language encoder (CLIP ablation)
# -------------------------------------------------------------------------

class SimpleTokenizer:
    """
    Minimal whitespace/punctuation tokenizer with a small vocab built from
    the provided texts. This replaces CLIP tokenize().
    """
    def __init__(self, texts: List[str], max_len: int = 32):
        ...
        # Build vocab from texts (very simple word tokenization)
        words = []
        for t in texts:
            words.extend(self._basic_tokenize(t))
    ...
    def _basic_tokenize(self, text: str) -> List[str]:
        ...
        # keep only alphanum as tokens; split on non-alphanum
    ...
    def encode(self, text: str) -> torch.Tensor:
        ...
        ids = [self.bos_id] + [self.stoi.get(t, self.unk_id) for t in toks] + [self.eos_id]
...
class SimpleTextEncoder(nn.Module):
    """
    Small trainable text encoder producing a sentence embedding.

    Architecture: token embedding -> BiGRU -> masked mean pool -> projection -> L2 norm
    Output dimension is set to 512 by default to mirror CLIP text emb size.
    """
    def __init__(
        self,
        vocab_size: int,
        ...
        self.tok_emb = nn.Embedding(vocab_size, token_dim, padding_idx=pad_id)
        self.gru = nn.GRU(
    ...
    def forward(self, token_ids: torch.Tensor) -> torch.Tensor:
        ...
        """
        x = self.tok_emb(token_ids)  # [B,L,D]
        h, _ = self.gru(x)           # [B,L,2D]

        # mask out padding for mean pool
        mask = (token_ids != self.pad_id).float()  # [B,L]
        denom = torch.clamp(mask.sum(dim=1, keepdim=True), min=1.0)  # [B,1]
        pooled = (h * mask.unsqueeze(-1)).sum(dim=1) / denom  # [B,2D]
...
# -------------------------------------------------------------------------
# 2. CalibDiff model with language conditioning (anisotropic, 2D param)
# -------------------------------------------------------------------------
...
class CalibDiff(nn.Module):
    def __init__(
        ...
        """
        CalibDiff with an extra language token, for 2D anisotropic params.

        Theta range is shared across all families: [1.0, 10.0].
        lang_dim: dimension of the (self-trained) text encoder sentence embeddings.
        """
        super().__init__()
        self.img_size = img_size
        self.patch_size = patch_size
        ...
        # Language projection: sentence embedding -> model dimension
        if self.use_lang:
    ...
    def forward(self, I_k, I_goal, tau_k, lang_emb):
        ...
        Args:
            ...
            lang_emb: [B,lang_dim] sentence embedding for the family description
        Returns:
            logits_x:   [B, n_bins]
            offsets_x:  [B, n_bins]
            logits_y:   [B, n_bins]
...
def eval_epoch_pde_multi(
    ...
    for fam, sim_idx in traj_keys:
        ...
        # language embedding for this family (self-trained encoder)
        fam_tok = fam2tokens[fam].unsqueeze(0).to(device)  # [1,L]
        lang_emb_fam = text_encoder(fam_tok)               # [1,lang_dim]
...
def _build_lang_batch(
    ...
    of self-trained language embeddings.
    """
    tok_list = [fam2tokens[fam] for fam in family_batch]
    tok_batch = torch.stack(tok_list, dim=0).to(device)  # [B,L]
    lang_batch = text_encoder(tok_batch)                 # [B,lang_dim]
    return lang_batch
...
def eval_epoch(
    ...
    for batch in dataloader:
        ...
        lang_batch = _build_lang_batch(family_batch, text_encoder, fam2tokens, device)
...
def train_epoch(
    ...
    fam2tokens: Dict[str, torch.Tensor],
    log_every: int = 50,
    curve_path: str = "train_loss_curve_multi_ani_textenc.png",
):
    model.train()
    text_encoder.train()
    total_loss = 0.0
...
def plot_all_losses(
    ...
    val_losses,
    pde_losses,
    out_path: str = "loss_curves_all_multi_ani_textenc.png",
):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss / MSE")
    plt.title("Training / Validation / PDE Loss Curves (multi anisotropic + TextEnc)")
    plt.grid(True, linestyle="--", alpha=0.3)
    plt.legend()
    plt.tight_layout()
...
# -------------------------------------------------------------------------
# 5. Main: multi-family anisotropic training + evaluation with self-trained text encoder
# -------------------------------------------------------------------------
...
def main():
    ...
    # ----------------------------------------------------------
    # Self-trained simple language encoder (CLIP ablation)
    # ----------------------------------------------------------
    all_texts = list(family_text.values())
    tokenizer = SimpleTokenizer(all_texts, max_len=32)

    fam2tokens: Dict[str, torch.Tensor] = {}
    for fam, text in family_text.items():
        fam2tokens[fam] = tokenizer.encode(text).detach().cpu()  # [L] long
    ...
    text_encoder = SimpleTextEncoder(
        ...
        token_dim=256,
        out_dim=512,   # keep 512 to mirror CLIP's ViT-B/32 text embedding size
        gru_layers=1,
        dropout=0.1,
    ).to(device)

    lang_dim = text_encoder.out_dim
    ...
    trainable_params = sum(p.numel() for p in model.parameters() if p.requires_grad) + sum(
        p.numel() for p in text_encoder.parameters() if p.requires_grad
    )
    print(f"Total parameters:     {total_params:,}")
    print(f"Trainable parameters: {trainable_params:,}")

    # IMPORTANT: optimizer now includes text_encoder params too
    ...
    for epoch in range(1, num_epochs + 1):
        train_loss = train_epoch(model, train_loader, optimizer, device, text_encoder, fam2tokens)
        val_loss = eval_epoch(model, val_loader, device, text_encoder, fam2tokens)
        ...
        plot_all_losses(
            ...
            val_loss_hist,
            pde_rgb_mse_hist,
            out_path="loss_curves_all_multi_ani_textenc.png",
        )

    torch.save(
        {
            "calibdiff_state_dict": model.state_dict(),
            "text_encoder_state_dict": text_encoder.state_dict(),
            "tokenizer_stoi": tokenizer.stoi,
            "tokenizer_max_len": tokenizer.max_len,
            "pad_id": tokenizer.pad_id,
        },
        "calibdiff_multi_ani_textenc_checkpoint.pt"
    )
    print("Training complete, model saved to calibdiff_multi_ani_textenc_checkpoint.pt")

    np.savez(
        "loss_histories_all_multi_ani_textenc.npz",
