...
class CalibDiffDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        I_k = torch.cat([rgb_k, phys_coords, pixel_idx], dim=0)
        I_goal = torch.cat([rgb_goal, phys_coords, pixel_idx], dim=0)
...
class CalibDiff(nn.Module):
    def __init__(
        ...
        self.patch_embed = PatchEmbed(
            in_ch=7, embed_dim=embed_dim, patch_size=patch_size, img_size=img_size
        )
