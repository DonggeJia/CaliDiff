...
def simulate_final_phi_isotropic(params, a_steps, nx=64, ny=64):
    ...
    for k in range(num_steps):
        ...
        while t < interval_end - 1e-12:
            ...
            Dy_phi = Dx_phi  # <-- same as Dx_phi for isotropy
...
class CalibDiff(nn.Module):
    ...
    def bins_to_param(self, bin_idx: torch.Tensor, offsets: torch.Tensor):
        ...
        # convert offset (fraction of bin width) to actual a offset
...
def _pde_worker_task(args):
    """
    Runs on a single CPU process:
    - simulate φ_hat with predicted a_hat(t)
    - simulate φ_true with ground-truth a_true(t)
    - compute concentration MSE and RGB MSE
    - compute param recovery flag
    """
    (
        sim_dir,
        params,
    ...
    # --- RGB MSE (pure NumPy + PIL, no torch) ---
    # predicted φ -> RGB
    ...
    param_success = bool(np.all(np.abs(a_hat_array - a_true_array) < param_tol))
...
def eval_epoch_pde(
    ...
    """
    Parallel physics-based evaluation on up to `max_sims` trajectories.

    Returns:
        mean_rgb_mse, mean_conc_mse, param_recovery_rate
    """
    from torch.utils.data import Subset
    ...
    # ------------------------------------------------------
    # Stage 1: use model (GPU/CPU) to predict a_hat(t) for each trajectory
    # ------------------------------------------------------
    tasks = []  # list of arguments for worker processes
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as ex:
        for conc_mse, rgb_mse, param_success in ex.map(_pde_worker_task, tasks):
...
#     distribution using predicted a(t) with the goal.
...
#         # ---------- ground-truth coefficients for this sim ----------
#         # assuming params["coefficients_ax"] has length >= K
#         a_true_array = np.array(params["coefficients_ax"], dtype=float)[:K]
...
#         # condition: for all k, |a_hat_k - a_true_k| < 0.1 * (theta_max - theta_min)
#         if np.all(np.abs(a_hat_array - a_true_array) < param_tol):
...
def train_epoch(
    ...
    device: torch.device,
    log_every: int = 50,
    curve_path: str = "train_loss_curve2_local_exp_iso.png",
):
    model.train()
    total_loss = 0.0
    n_batches = 0
    ...
    for step, batch in enumerate(dataloader, start=1):
        ...
        # 3) combine
        loss = loss_cls + 0.1 * loss_reg      # choose λ, e.g. 1.0 or 0.1 depending on scale
...
def eval_epoch(
    ...
    for batch in dataloader:
        ...
        target_params = batch["target_params"].to(device)  # [B,1], continuous a
    ...
    return total_loss / n_batches  # you can also return cls/reg separately if you like
...
def plot_all_losses(
    ...
    val_losses,
    pde_losses,
    out_path: str = "loss_curves_all_exp_iso.png",
):
    """
...
def main():
    root_dir = "/data/IMcali/sim_results_2025-12-19 18:25:27_exp_iso"  # TODO: set your dataset path
    img_size = 128
    patch_size = 2
    batch_size = 16
    num_epochs = 100
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        pde_rgb_mse, conc_mse, param_rec = eval_epoch_pde(
            ...
            val_set,
            device,            
            max_sims=50,       # or None for all trajectories
            nx_sim=64,
            ny_sim=64,
        )
        ...
        print(
            f"Epoch {epoch:03d} | "
            f"train loss: {train_loss:.4f} | val loss: {val_ce_loss:.4f} | "
            f"PDE RGB MSE: {pde_rgb_mse:.6f} | "
            f"Conc MSE: {conc_mse:.6e} | "
            f"Param recovery: {param_rec*100:.2f}%"
        )

        # keep plotting the same three curves as before (or extend later if you want)
        plot_all_losses(
            train_loss_hist,
            val_loss_hist,
            pde_rgb_mse_hist,
            out_path="loss_curves_all_local_lin_iso.png",
        )
    ...
    np.savez(
        "loss_histories_all_local_exp_iso.npz",
