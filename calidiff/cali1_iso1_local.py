...
def eval_epoch_pde(
    ...
    for sim_idx in sim_idx_list:
        ...
        for k in range(K):
            phi_k_path    = os.path.join(sim_dir, f"phi_interval_{k:02d}.png")
            phi_goal_path = os.path.join(sim_dir, f"phi_interval_{K:02d}.png")  # not actually needed here
...
def train_epoch(
    ...
    device: torch.device,
    log_every: int = 50,
    curve_path: str = "train_loss_curve2_local.png",
):
    model.train()
    total_loss = 0.0
    n_batches = 0
...
def main():
    root_dir = "/data/IMcali/sim_results_2025-12-16 21:16:41"  # TODO: set your dataset path
    img_size = 128
    patch_size = 2
    batch_size = 16
    num_epochs = 100
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        plot_all_losses(
            ...
            val_loss_hist,
            pde_loss_hist,
            out_path="loss_curves_all_local.png",
        )
